function varargout = NPMold( s, t, E, Is, R)
% Returns:
...
st2 = [s(:), t(:)]';
E = templateSort(E, st1, st2);
Is = templateSort(Is, st1, st2);
R = templateSort(R, st1, st2);
B = incidence(grph);
% plot(grph,'NodeLabel',1:height(grph.Nodes))
An = inc2nc(B);
% =================================================
...
object1 = [ num1(:), [phi(:); 0] ];
varargout{1} = array2table(object1, 'VariableNames', {'Num';'Phi'});
if nargout == 2;
    num2 = 1:length(s);
    object2 = [num2(:), s(:), t(:), R(:), E(:), Is(:), U(:), I(:)];
    varargout{2} = array2table(object2, 'VariableNames', {'Num';'s';'t';'R';'E'; 'Is'; 'U'; 'I'});
end
